...
class JsonRpcChecks:
    ...
    def __init__(
        self, card: AgentCard, jsonrpc_url: str, client: JsonRpcClient
    ) -> None:
        self.card = card
        ...
        try:
            ...
            if isinstance(parsed, JSONRPCErrorResponse):
                ...
                msg = (
                    ...
                    if ok
                    else f"unexpected error code {code}"
                )
                return [
                    CheckResult(
                        rule="RPC-001",
                ...
                return [
                    CheckResult(
                        rule="RPC-001",
                        ok=False,
                        message="expected error for unknown method",
    ...
    def _message_send_roundtrip(self) -> Tuple[List[CheckResult], Optional[str]]:
        ...
        try:
            parsed, raw = self.client.call_and_parse(
            ...
            return [
                CheckResult(
                    rule="RPC-010",
                    ok=False,
                    message=f"message/send failed {e}",
                    severity=Severity.ERROR,
                )
            ], None
        ...
        r_hdr = CheckResult(
            ...
            severity=Severity.ERROR if not hdr_ok else Severity.INFO,
        )
        ...
        if isinstance(parsed, JSONRPCErrorResponse):
            return [
                CheckResult(
                    rule="RPC-010",
                    ok=False,
                    message=f"message/send error {parsed.error.code} {parsed.error.message}",
                    severity=Severity.ERROR,
                ),
                r_hdr,
            ], None
        ...
        try:
            ...
            return [
                CheckResult(
                    rule="RPC-010",
                    ok=False,
                    message=f"unexpected success type {type(parsed).__name__}",
                    severity=Severity.ERROR,
                ),
                r_hdr,
            ], None
        ...
        if kind == "message":
            return [
                CheckResult(
                    rule="RPC-010",
                    ok=True,
                    message="message/send returned Message",
        ...
        if kind == "task":
            ...
            return [
                CheckResult(
                    rule="RPC-010",
                    ok=True,
                    message="message/send returned Task",
                    severity=Severity.INFO,
                ),
                r_hdr,
            ], tid if ok else None
        ...
        return [
            CheckResult(
                rule="RPC-010",
                ok=False,
                message=f"unexpected result shape (type={type_info}, kind={kind})",
                severity=Severity.ERROR,
            ),
            r_hdr,
        ], None
    ...
    def _tasks_get(self, task_id: str) -> List[CheckResult]:
        req = GetTaskRequest(
            id="2", params=TaskQueryParams(id=task_id, history_length=1)
        )
        try:
            parsed, _ = self.client.call_and_parse(
                req.model_dump(mode="json", exclude_none=True)
            )
        except Exception as e:
            return [
                CheckResult(
                    rule="RPC-020",
                    ok=False,
                    message=f"tasks/get transport error {e}",
                    severity=Severity.ERROR,
                )
            ]
        ...
        if isinstance(parsed, JSONRPCErrorResponse):
            return [
                CheckResult(
                    rule="RPC-020",
                    ok=False,
                    message=f"tasks/get error {parsed.error.code} {parsed.error.message}",
                    severity=Severity.ERROR,
                )
            ]
        ...
        return [
            CheckResult(
                rule="RPC-020",
                ok=ok,
                message="tasks/get returned Task"
                if ok
                else f"tasks/get unexpected result (kind={kind}, id={rid})",
                severity=Severity.ERROR if not ok else Severity.INFO,
            )
        ]
    ...
    def _tasks_cancel(self, task_id: str) -> List[CheckResult]:
        ...
        try:
            ...
            return [
                CheckResult(
                    rule="RPC-021",
                    ok=False,
                    message=f"tasks/cancel transport error {e}",
                    severity=Severity.ERROR,
                )
            ]
        ...
        if isinstance(parsed, JSONRPCErrorResponse):
            ...
            return [
                CheckResult(
                    rule="RPC-021",
                    ok=False,
                    message=f"tasks/cancel error {code} {parsed.error.message}",
                    severity=Severity.ERROR,
                )
            ]
        ...
        return [
            CheckResult(
                rule="RPC-021",
                ok=ok,
                message="tasks/cancel returned Task"
                if ok
                else f"tasks/cancel unexpected result (kind={kind})",
                severity=Severity.ERROR if not ok else Severity.INFO,
            )
        ]
    ...
    def _streaming_and_resubscribe(self) -> List[CheckResult]:
        ...
        if streaming is False:
            return [
                CheckResult(
                    rule="RPC-030",
                    ok=True,
                    message="streaming not declared; skipping stream test",
                    severity=Severity.INFO,
                )
            ]
        try:
            ...
            return [
                CheckResult(
                    rule="RPC-030",
                    ok=False,
                    message=f"message/stream failed {e}",
                    severity=Severity.ERROR,
                )
            ]
        ...
        results = [
            CheckResult(
                rule="RPC-030",
                ok=ok_any,
                message="received SSE events" if ok_any else "no SSE events received",
                severity=Severity.ERROR if not ok_any else Severity.INFO,
            )
        ]
        if not ok_any:
            return results
        ...
        if not tid:
            results.append(
                CheckResult(
                    rule="RPC-031",
                    ok=False,
                    message="could not extract taskId from SSE events; skipping resubscribe",
        ...
        try:
            ...
            results.append(
                CheckResult(
                    rule="RPC-032",
                    ok=ok_re,
                    message="tasks/resubscribe yielded events"
                    if ok_re
                    else "tasks/resubscribe yielded no events",
                    severity=Severity.ERROR if not ok_re else Severity.INFO,
                )
            )
        except Exception as e:
            results.append(
                CheckResult(
                    rule="RPC-032",
                    ok=False,
                    message=f"tasks/resubscribe failed {e}",
                    severity=Severity.ERROR,
                )
            )
        return results
    ...
    def _push_notifications(self, task_id: str) -> List[CheckResult]:
        ...
        try:
            ...
            return [
                CheckResult(
                    rule="RPC-040",
                    ok=False,
                    message=f"pushNotification set transport error {e}",
                    severity=Severity.ERROR,
                )
            ]
        ...
        results.append(
            CheckResult(
                rule="RPC-040",
                ok=True,
                message="pushNotificationConfig set OK",
                severity=Severity.INFO,
            )
        )
        ...
        get_payload = {
            ...
            "params": {"id": task_id}
            | ({"pushNotificationConfigId": pn_id} if pn_id else {}),
        }
        try:
            ...
            if isinstance(gparsed, JSONRPCErrorResponse):
                ...
                if gparsed.error.code in (-32003, -32601):
                    results.append(
                        CheckResult(
                            rule="RPC-041",
                            ok=True,
                            message=f"push get not supported ({gparsed.error.code})",
                            severity=Severity.INFO,
                        )
                    )
                else:
                    results.append(
                        CheckResult(
                            rule="RPC-041",
                            ok=False,
                            message=f"push get error {gparsed.error.code} {gparsed.error.message}",
                            severity=Severity.ERROR,
                        )
                    )
            else:
                results.append(
                    CheckResult(
                        rule="RPC-041",
                        ok=True,
                        message="pushNotificationConfig get OK",
                        severity=Severity.INFO,
                    )
                )
        except Exception as e:
            results.append(
                CheckResult(
                    rule="RPC-041",
                    ok=False,
                    message=f"push get transport error {e}",
                    severity=Severity.ERROR,
                )
            )
        ...
        try:
            ...
            if isinstance(lparsed, JSONRPCErrorResponse):
                if lparsed.error.code in (-32003, -32601):
                    results.append(
                        CheckResult(
                            rule="RPC-042",
                            ok=True,
                            message=f"push list not supported ({lparsed.error.code})",
                            severity=Severity.INFO,
                        )
                    )
                else:
                    results.append(
                        CheckResult(
                            rule="RPC-042",
                            ok=False,
                            message=f"push list error {lparsed.error.code} {lparsed.error.message}",
                            severity=Severity.ERROR,
                        )
                    )
            else:
                results.append(
                    CheckResult(
                        rule="RPC-042",
                        ok=True,
                        message="pushNotificationConfig list OK",
                        severity=Severity.INFO,
                    )
                )
        except Exception as e:
            results.append(
                CheckResult(
                    rule="RPC-042",
                    ok=False,
                    message=f"push list transport error {e}",
                    severity=Severity.ERROR,
                )
            )
        ...
        if pn_id:
            ...
            try:
                ...
                if isinstance(dparsed, JSONRPCErrorResponse):
                    if dparsed.error.code in (-32003, -32601):
                        results.append(
                            CheckResult(
                                rule="RPC-043",
                                ok=True,
                                message=f"push delete not supported ({dparsed.error.code})",
                                severity=Severity.INFO,
                            )
                        )
                    else:
                        results.append(
                            CheckResult(
                                rule="RPC-043",
                                ok=False,
                                message=f"push delete error {dparsed.error.code} {dparsed.error.message}",
                                severity=Severity.ERROR,
                            )
                        )
                else:
                    results.append(
                        CheckResult(
                            rule="RPC-043",
                            ok=True,
                            message="pushNotificationConfig delete OK",
                            severity=Severity.INFO,
                        )
                    )
            except Exception as e:
                results.append(
                    CheckResult(
                        rule="RPC-043",
                        ok=False,
                        message=f"push delete transport error {e}",
                        severity=Severity.ERROR,
                    )
                )
        else:
            results.append(
                CheckResult(
                    rule="RPC-043",
                    ok=True,
                    message="push delete skipped (no id returned by server)",
                    severity=Severity.INFO,
                )
            )
        ...
        if not saec:
            return [
                CheckResult(
                    rule="RPC-050",
                    ok=True,
                    message="extended card not declared; skipping",
                    severity=Severity.INFO,
                )
            ]
        ...
        try:
            ...
            return [
                CheckResult(
                    rule="RPC-050",
                    ok=False,
                    message=f"extended card transport error {e}",
                    severity=Severity.ERROR,
                )
            ]
        ...
        if sc in (401, 403):
            return [
                CheckResult(
                    rule="RPC-050",
                    ok=True,
                    message=f"extended card returned HTTP {sc} (expected without auth)",
                    severity=Severity.INFO,
                )
            ]
        ...
        try:
            ...
            return [
                CheckResult(
                    rule="RPC-050",
                    ok=False,
                    message="extended card response not JSON",
                    severity=Severity.ERROR,
                )
            ]
            ...
            try:
                ...
                return [
                    CheckResult(
                        rule="RPC-050",
                        ok=False,
                        message=f"extended card JSON-RPC error {err.error.code} {err.error.message}",
                        severity=Severity.ERROR,
                    )
                ]
            except Exception:
                return [
                    CheckResult(
                        rule="RPC-050",
                        ok=False,
                        message="extended card invalid JSON-RPC payload",
                        severity=Severity.ERROR,
                    )
                ]
        ...
        if isinstance(parsed, JSONRPCErrorResponse):
            return [
                CheckResult(
                    rule="RPC-050",
                    ok=False,
                    message=f"extended card error {parsed.error.code} {parsed.error.message}",
                    severity=Severity.ERROR,
                )
            ]
        ...
        if not isinstance(res, dict):
            return [
                CheckResult(
                    rule="RPC-050",
                    ok=False,
                    message="extended card result not an object",
                    severity=Severity.ERROR,
                )
            ]
        ...
        try:
            ...
            return [
                CheckResult(
                    rule="RPC-050",
                    ok=True,
                    message="extended card OK (validated AgentCard)",
                    severity=Severity.INFO,
                )
            ]
            ...
            return [
                CheckResult(
                    rule="RPC-050",
                    ok=False,
                    message=f"extended card invalid AgentCard: {ve}",
                    severity=Severity.ERROR,
                )
            ]
