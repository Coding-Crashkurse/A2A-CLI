...
class CardService:
    ...
    def fetch_raw(self, target: str, override_card_url: str | None) -> Tuple[str, Dict[str, Any], List[CheckResult]]:
        ...
        try:
            ...
            ok_origin = True
            results.append(CheckResult(rule="NET-001", ok=True, message=f"Origin reachable HTTP {resp_origin.status_code}", severity=Severity.INFO))
        except Exception as e:
            results.append(CheckResult(rule="NET-001", ok=False, message=f"Origin not reachable: {e}", severity=Severity.ERROR))
        ok_card_url = False
        raw: Dict[str, Any] = {}
        try:
            resp = self.http.get(card_url)
            ok_card_url = True
            results.append(CheckResult(rule="URL-001", ok=True, message=f"Card endpoint reachable HTTP {resp.status_code}", severity=Severity.INFO))
            ...
            results.append(CheckResult(rule="URL-001", ok=False, message=f"Card endpoint not reachable: {e}", severity=Severity.ERROR))
    ...
    def parse(self, raw: Dict[str, Any]) -> Tuple[AgentCard | None, List[CheckResult]]:
        ...
        try:
            ...
            results.append(CheckResult(rule="CARD-STRUCT", ok=True, message="AgentCard parsed via schema", severity=Severity.INFO))
            return card, results
        except ValidationError as ve:
            results.append(CheckResult(rule="CARD-STRUCT", ok=False, message=f"Schema validation failed: {ve.errors()}", severity=Severity.ERROR))
            return None, results
